...
FuguTTP::FuguTTP(const WebSocketClient & client,
                 const string & abr_name, const YAML::Node & abr_config)
  : Fugu(client, abr_name, abr_config)
{
  ...
  if (abr_config["model_dir"]) {
    ...
    if (abr_name == "fugu_ttp_mle") {
      is_mle_= true;
    }

    if (abr_name == "fugu_ttp_no_tcp_info") {
      ttp_input_dim_ = 17;
      no_tcp_info_ = true;
    }
  } else {
    throw runtime_error("Fugu requires specifying model_dir in abr_config");
  }
}

void FuguTTP::normalize_in_place(size_t i, vector<double> & input)
{
...
void FuguTTP::reinit_sending_time()
{
