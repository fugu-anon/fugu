...
static fs::path src_path;  /* path to fugu/src directory */
...
int retrieve_expt_id(const string & json_str)
{
  try {
    ...
    create_table.exec("CREATE TABLE IF NOT EXISTS fugu_experiment "
    ...
    db_conn.prepare("select_id",
      "SELECT id FROM fugu_experiment WHERE hash = $1;");
    db_conn.prepare("insert_json",
      "INSERT INTO fugu_experiment (hash, data) VALUES ($1, $2) RETURNING id;");
