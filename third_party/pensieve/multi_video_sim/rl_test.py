...
def get_fugu_info(sock):
    json_len = sock.recv(2, socket.MSG_WAITALL)
    try:
        json_len_num = struct.unpack("!H", json_len)[0]
    except Exception:
        print "Failed to decode info from Fugu over IPC"
        sys.exit(1)
    json_data = sock.recv(json_len_num, socket.MSG_WAITALL)
    fugu_info = json.loads(json_data)
    delay = fugu_info['delay'];
    playback_buf = fugu_info['playback_buf'];
    rebuf_time = fugu_info['rebuf_time'];
    last_chunk_size = fugu_info['last_chunk_size'];
    next_chunk_size = fugu_info['next_chunk_sizes'];

    return delay, playback_buf, rebuf_time, last_chunk_size, next_chunk_size


def send_fugu_next_action(sock, bit_rate):
    bit_rate_dict = {}
...
def main():
    ...
    with tf.Session() as sess:
        ...
        while True:  # serve video forever
            ...
            # Now I have my action! Send this action back to the Fugu server over IPC
            send_fugu_next_action(fugu_sock, bit_rate)
